/* 
   ...
   etc) according to their colonization/extinction values. At different levels of 
   similarity on the model parameter space, partitions, so different species groupings, 
   ...
   computationally-intensive work from the library: libda_AUX.a:
   ...
   ~$ ./upgma_model_selection_MacKenzie_3ATOLLS -Nr 3 -Nc 28 -pC 1.0 -pE 1.0 -sP 4 -sT 1.0E-08 -m0 0.0 -M0 10.0 -m1 0.0 -M1 10.0
   ...
   ~$ valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --verbose --log-file=valgrind-out.txt ./upgma_model_selection_MacKenzie_3ATOLLS -Nr 3 -Nc 28 -pC 1.0 -pE 1.0 -sP 4 -sT 1.0E-08 -m0 0.0 -M0 10.0 -m1 0.0 -M1 10.0
*/
#include <MODEL.h>
...
int main(int argc, char **argv)
{
  ...
  */
  assert( No_of_SPECIES == 3  ); //-Nr 3 No of files (corresponding to each atoll)
  ...
  p = strcat (Species_Tag[2], "Kavratti");
  ...
  for (i=0; i<No_of_ROWS_MAX; i++)
    Species_Name[i] = (char *)calloc(100, sizeof(char) );
